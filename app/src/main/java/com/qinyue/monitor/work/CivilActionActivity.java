...
public class CivilActionActivity extends BaseActivity {
        ...
        switch (view.getId()) {
            case R.id.btn_submit: {//提交
                if (nameEdit.getText().toString().trim().isEmpty()){
                    XToast.info(CivilActionActivity.this,"请输入申诉人姓名或单位名称").show();
                    break;
                }
                if (yagxTv.getText().toString().trim().isEmpty()){
                    XToast.info(CivilActionActivity.this,"请选择申诉人与原案关系").show();
                    break;
                }
                if (zjlxTv.getText().toString().trim().isEmpty()){
                    XToast.info(CivilActionActivity.this,"请选择申诉人证件类型").show();
                    break;
                }
                if (zjhmEdit.getText().toString().trim().isEmpty()){
                    XToast.info(CivilActionActivity.this,"请输入申诉人证件号码").show();
                    break;
                }
                if ("居民身份证".equals(zjlxTv.getText().toString().trim())&&!RegexUtils.isIDCard18(zjhmEdit.getText().toString().trim())){
                    XToast.info(CivilActionActivity.this,"证件号码错误").show();
                    break;
                }
                if (szdEdit.getText().toString().trim().isEmpty()){
                    XToast.info(CivilActionActivity.this,"请输入您的居所地").show();
                    break;
                }
                if (phoneEdit.getText().toString().trim().isEmpty()){
                    XToast.info(CivilActionActivity.this,"请输入您的电话号码").show();
                    break;
                }
                ...
                if (bYagxTv.getText().toString().trim().isEmpty()){
                    XToast.info(CivilActionActivity.this,"请选择被申诉人与原案关系").show();
                    break;
                }
                if (bGzdwEdit.getText().toString().trim().isEmpty()){
                    XToast.info(CivilActionActivity.this,"请输入被申诉人工作单位全称").show();
                    break;
                }
                if (afdTv.getText().toString().trim().isEmpty()){
                    XToast.info(CivilActionActivity.this,"请选择案发地").show();
                    break;
                }
                if (contentTv.isEmpty()){
                    XToast.info(CivilActionActivity.this,"请输入案情摘要").show();
                    break;
                }
            ...
            case R.id.rl_afd: {//案发地
                Intent intent = new Intent(CivilActionActivity.this, ChooseAfdActivity.class);
        ...
        Disposable subscribe = RxHttp.postForm(TagConstant.BASEURL + NetConstant.getTypeCode)
                ...
                .subscribe(s -> {
                    ...
                    if (s.getResult() == 200) {
                        ...
                        XToast.error(CivilActionActivity.this, s.getMsg()).show();
                    }
                }, throwable -> {
                    miniLoadingDialog.dismiss();
                    XToast.error(CivilActionActivity.this, throwable.getMessage()).show();
                });
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == PictureConfig.CHOOSE_REQUEST || requestCode == PictureConfig.REQUEST_CAMERA) {
            ...
            if (selectList != null && selectList.size() > 0) {
                switch (clIndex) {
                    case 0: {//民行监督申请书
                        Glide.with(CivilActionActivity.this).load(selectList.get(0).getPath()).into(mhjdImg);
                    }
                    break;
                    case 1: {//法律文书
                        Glide.with(CivilActionActivity.this).load(selectList.get(0).getPath()).into(wsImg);
                    }
                    break;
                    case 2: {//身份证
                        Glide.with(CivilActionActivity.this).load(selectList.get(0).getPath()).into(sfzImg);
                    }
                    break;
                    case 3: {//其他
                        Glide.with(CivilActionActivity.this).load(selectList.get(0).getPath()).into(qtImg);
                    }
                    break;
                }
        ...
        new BottomSheet.BottomListSheetBuilder(this)
                ...
                .setOnSheetItemClickListener(new BottomSheet.BottomListSheetBuilder.OnSheetItemClickListener() {
                    ...
                    public void onClick(BottomSheet dialog, View itemView, int position, String tag) {
                        dialog.dismiss();
                        if (position == 0) {
                            PictureSelector.create(CivilActionActivity.this)
        ...
        Disposable subscribe = RxHttp.postForm(TagConstant.BASEURL + NetConstant.fileUpload)
                ...
                .subscribe(s -> {
                    if (s.getResult()==200){
                        ...
                        if (retry==3){
                            miniLoadingDialog.dismiss();
                            XToast.error(CivilActionActivity.this,"第"+(index+1)+"张图片上传失败,请检查").show();
                        }else {
                            upLoadFiles(index);
                        }
                    }
    ...
    private void submit() {
        ...
        Disposable subscribe = RxHttp.postForm(TagConstant.BASEURL + NetConstant.personAppeal)
                .add("appId", TagConstant.APPID)
                .add("code", TagConstant.CODE)
                .add("data", aes)
                ...
                .subscribe(s -> {
                    miniLoadingDialog.dismiss();
                    if (s.getResult()==200){
                        XToast.success(CivilActionActivity.this,s.getMessage()).show();
                        finish();
                    }else {
                        XToast.error(CivilActionActivity.this,s.getMessage()).show();
                    }
                }, throwable -> {
                    miniLoadingDialog.dismiss();
                    XToast.error(CivilActionActivity.this,throwable.getMessage()).show();
